...
app.post("/successSubs", (req, res) => {
    ...
    if (expectedSignature == req.body.razorpay_signature) {
        ...
        clubUsers.findOneAndUpdate({ _id: req.session.userid }, { $set: { subId: req.body.razorpay_subscription_id } }, { new: true },
            (err, result) => {
                if (err) {
                    console.log(err);
                } else {
...
app.get("/chats/:username", (req, res) => {
    ...
    res.render("chats")
        ...
        // var senderroom = req.session.username
        // console.log(sender)
        // const receiversid = req.params.username
...
io.on('connection', function(socket) {
    ...
    //attach incoming listner for new users
    ...
    socket.on("user_connected", function(username) {
            ...
            // socket id will be used to send msg to individual users
...
app.post("/chats", (req, res) => {
    var chatbox = req.body.chatbox
    console.log(chatbox)
    const receiversid = req.params.id
    console.log(receiversid)
    var sender = req.session.userid
    console.log(sender)
    clubUsers.findOne({ _id: sender }).exec((err, results) => {
    ...
    // clubUsers.findOne({ _id: receiversid }).exec((err, results) => {

    //     results.recieved.push({
    //         user: sender,
    //         msggot: chatbox
    //     });
...
http.listen(process.env.PORT || 2022, function() {
